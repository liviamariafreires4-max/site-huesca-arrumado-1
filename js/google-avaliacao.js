(() => {
  ...
  const mostrarFallback = (
    mensagem = "Clique para ver as avaliações atualizadas",
  ) => {
  ...
  const chaveNaoConfigurada =
    !GOOGLE_MAPS_API_KEY || GOOGLE_MAPS_API_KEY === "COLE_SUA_CHAVE_AQUI";
  ...
  window.iniciarAvaliacaoGoogle = async () => {
    try {
      ...
      mostrarFallback(
        "Não foi possível carregar a nota agora. Clique para abrir no Google.",
      );
    }
  };
  ...
  script.onerror = () =>
    mostrarFallback(
      "Falha ao conectar ao Google. Clique para abrir as avaliações.",
    );
